...
-- 조인 
-- DEPARTMENT 테이블 -> DEPARTMENT_NAME
-- EMPLOYEE 테이블 -> EMPLOYEE_ID, FIRST_NAME, DEPARTMENT_ID
...
-- 서브쿼리 : 최대연봉
-- 메인쿼리 : SELECT 칼럼 FROM 테이블 WHERE 연봉 > (직업='IT_PROG' 최대연봉)
...
-- MANAGER가 아닌 사원 : EMPLOYEE_ID 가 MANAGER_ID에 없는 사원 
-- 서브쿼리 : MANAGER_ID의 중복을 제거 결과(MANAGER의 번호 모아두기)

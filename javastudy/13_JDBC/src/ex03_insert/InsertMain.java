...
public class InsertMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// DB 접속 - Connnection 객체 생성
			String url = "jdbc:oracle:thin:@localhost:1521:xe";
			String user = "SCOTT";
			String password = "TIGER";
			con = DriverManager.getConnection(url, user, password);
			
			// 쿼리문 작성(뱐수 처리할 부분은 ?로 처리)
			...
			// 쿼리문에 포함된 ?에 변수 전달하기
			// 쿼리문에 작성된 ?의 순서대로 값을 채워줌
			ps.setString(1, board.getTitle());	 // 1번째 ?에 board.getTitle() 전달하기(setString : title이 String이므로)
			ps.setString(2, board.getContent()); // 2번째 ?에 board.getConten() 전달하기(setString : content가 String이므로)

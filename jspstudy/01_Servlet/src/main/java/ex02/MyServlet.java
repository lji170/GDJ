...
public class MyServlet extends HttpServlet {
 ...
 protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 2) HttpServiletRequest request 객체가 처리 (Tomacat이 있어야 사용 가능)
        
        // 1) 요청에 포함된 한글 처리(문자셋 : UTF-8)
        request.setCharacterEncoding("UTF-8");
        
        // 2) 요청 파라미터(Parmeter) 확인
        // (1)URL?파라미터=값
        ...
        // 2. 응답
        //	  1) 서버 -> 클라이언트로 보내는 응답	
        //    2) HttpServleResponse response 객체가 처리 (Tomact이 있어야 사용 가능)
        ...
        //        -write() 메소드보다는 print()/println()메소드에 사용하는 것이 권장
        PrintWriter out = response.getWriter();
        
        // 4) 응답 만들기 (HTML 문서 만들기)
        out.println("<htmllang=\"ko\">");
        out.println("<head>");
